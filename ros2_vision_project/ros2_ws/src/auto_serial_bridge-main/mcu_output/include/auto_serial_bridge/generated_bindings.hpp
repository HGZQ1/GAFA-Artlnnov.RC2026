...
inline void register_all(SerialController* node) {
    ...
    node->add_subscription(node->create_subscription<geometry_msgs::msg::Twist>(
        ...
        [node](const geometry_msgs::msg::Twist::SharedPtr msg) {
            Packet_ClimbCmd pkt;
            pkt.climb_mode = msg->linear.x;
            pkt.slope_deg = msg->linear.y;
            pkt.speed_factor = msg->linear.z;
            pkt.torque_factor = msg->angular.x;
...
struct ProtocolPublishers {
    ...
    rclcpp::Publisher<geometry_msgs::msg::Twist>::SharedPtr pub_EncoderFeedback;
    ...
    void init(SerialController* node) {
        ...
        pub_EncoderFeedback = node->create_publisher<geometry_msgs::msg::Twist>("/feedback/encoder", 10);
    }
};
...
inline void dispatch_packet(ProtocolPublishers& pubs, uint8_t id, const std::vector<uint8_t>& data) {
    switch(id) {
        ...
        case PACKET_ID_ENCODERFEEDBACK: {
            if (data.size() != sizeof(Packet_EncoderFeedback)) break;
            const Packet_EncoderFeedback* pkt = reinterpret_cast<const Packet_EncoderFeedback*>(data.data());
