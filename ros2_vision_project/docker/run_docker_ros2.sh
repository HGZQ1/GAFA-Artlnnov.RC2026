...
build_image() {
    local mode=$1
    # 提取构建参数，如果没传 GPU_SERIES，默认为 40
    local gpu_series=${GPU_SERIES:-40}
    
    print_info "构建ROS2镜像: $mode (GPU系列: $gpu_series)"
    ...
    case "$mode" in
        cpu)
            ...
            ;;
        gpu)
            # 关键修改：将 gpu_series 变量通过 --build-arg 传给 Dockerfile
            GPU_SERIES=$gpu_series docker compose -f docker-compose.ros2.yml build --build-arg GPU_SERIES=$gpu_series yolov8-ros2-gpu
            ;;
        jetson)
